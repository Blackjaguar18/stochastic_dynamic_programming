defmodule SDP.FirstWorker do
  ...
  def child_spec(arg) do
    %{id: :initial_worker, start: {SDP.FirstWorker, :start_link, arg}}
  end
  ...
  def handle_call({:start, worker}, _from, _state) do
    ...
    state = SDP.bellman_initial(future_optimal_values)
  ...
  values, used to display the final result in the case of this worker, as it is
  the last computational stage of the procedure.
  """
  ...
  @doc """
  Endpoint to reset the state of the process, resetting its state and thus removing
  the bellman values from its state.
  """
